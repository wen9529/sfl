...
start_marker = "// 5. /stats 或 /profit 或 /pnl 动态累计盈亏报表"
if start_marker not in code:
    start_marker = "// 5. /stats"
...
if start_pos != -1:
    # find where this if block ends (next return; or end of function)
    sub = code[start_pos:]
    end_pos = start_pos + sub.find("return;\n        }") + len("return;\n        }")
    code = code[:start_pos] + new_stats_block + code[end_pos:]
    with open("bot_commands.php", "w", encoding="utf-8") as f:
        f.write(code)
    print("BOT COMMANDS WEEKLY STATS UPDATED SUCCESSFULLY")
else:
