...
async function run() {
  ...
  for (let i = 0; i < 50; i++) {
    // To predict draws[i], we give the algorithm draws[i+1] and older.
    const sub = draws.slice(i + 1);
    const pred = generate50DrawsPrediction(sub);
    const actualCodes = draws[i].openCode.split(',').map(Number);
  ...
  console.log(`[Real 50-Draw Backtest]`);
